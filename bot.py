...
async def start_handler(c: Client, m: Message):
    ...
    if m.from_user.id != int(Config.OWNER):
        ...
        user.set()
    res = await m.reply_text(
        text=f"<b>нєу</b> {m.from_user.first_name},\n\n<b><i>➻ I am a video/File Merger Bot that seamlessly merges multiple files and uploads them to Telegram and Drive.</i></b>\n───────────────────\n<i>Check /about for more..</i> ",
        quote=True,
    )
    del user
...
async def makeButtons(bot: Client, m: Message, db: dict):
    ...
    markup.append([InlineKeyboardButton("🗑️ Clear Files", callback_data="cancel")])
    return markup
